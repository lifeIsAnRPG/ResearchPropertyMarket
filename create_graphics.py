...
def create_line_fig(chosen_segment, theme_status):
    ...
    lines_fig = go.Figure()
    fig_new = go.Scatter(x=dynamics_data.query("city == @chosen_segment")['Даты'], y=dynamics_data.query("city == @chosen_segment")['Квартиры в новостройке, за м2, руб.'],
                        mode='lines+markers+text',
                        name='Первичное',
                        text=dynamics_data.query("city == @chosen_segment")['Изменение'])
    fig_secondary = go.Scatter(x=dynamics_data.query("city == @chosen_segment")['Даты'], y=dynamics_data.query("city == @chosen_segment")['Квартиры вторичка, за м2, руб.'],
                        mode='lines+markers+text',
                        name='Вторичное',
                        text=dynamics_data.query("city == @chosen_segment")['Изменение.1'])
    lines_fig.add_trace(fig_new)
    lines_fig.add_trace(fig_secondary)
    lines_fig.update_traces(textposition="bottom right")
